...
if _ist_hour >= 23 or _ist_hour < 7:
    if _disable_quiet_hours:
        st.info(
            f"🔓 **Evaluation Mode Active ({_ist_hour:02d}:xx IST)** — "
            "It's currently within Quiet Hours (11 PM–7 AM IST), but `DISABLE_QUIET_HOURS=true` is set in `.env`. "
        ...
        st.warning(
            ...
            "We don't want to disturb customers between **11 PM and 7 AM IST**. "
...
with tab_ops:
    ...
    with col_rules:
        ...
        with st.container(border=True):
            st.markdown("🚫 **Quiet Hours Guard:** Delayed action between `11 PM – 7 AM IST` for notification compliance.")
